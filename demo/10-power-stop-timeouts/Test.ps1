...
try {
  ...
  # No stop_method/timeouts in provider v2; ensure basic lifecycle still works
  ...
  if ($vmRes) { Write-Ok 'state resource present' } else { $failures += 'No hypervapiv2_vm in state'; Write-Err 'No hypervapiv2_vm in state' }
