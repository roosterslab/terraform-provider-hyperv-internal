...
try {
  # Apply
  Write-Info 'Applying demo with switch_name'
  & (Join-Path $PSScriptRoot 'Run.ps1') -Endpoint $Endpoint -VmName $VmName -SwitchName $SwitchName -BuildProvider:$BuildProvider | Write-Host

  # Outputs
  $out = terraform output -json | ConvertFrom-Json
  $osPath = $out.os_disk_path.value
  if ($out.switch_name.value -ne $SwitchName) { $failures += 'switch_name output mismatch'; Write-Warn 'switch_name output mismatch' } else { Write-Ok ("switch_name: {0}" -f $SwitchName) }
  ...
  if ($vmRes) {
    $vals = $vmRes.values
    if ($vals.switch_name -ne $SwitchName) { $failures += 'state.switch_name mismatch'; Write-Err 'state.switch_name mismatch' } else { Write-Ok 'state.switch_name ok' }
